...
- **Karar:** `managers/ttl_cache.py::DiskTTLCache` — `TTLCache` ile aynı arayüz, disk
  ...
  her işlem diski okur (`ttl_cache.py:82-93`). `PROCESS_CACHE` bunun üzerine
  `adopt_file_fields=("path", "original_path")` ile kurulur (`routes/processing.py:68-72`)
...
- **Gerekçe:** Docstring'den birebir: "İşlem hacmi kullanıcı-eylemi mertebesinde (dakikada
  ...
  yalnız biri kazanır (`ttl_cache.py:99-102`).
