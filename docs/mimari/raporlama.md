...
| Uçlar | `backend/routes/reports.py` | `/api/reports/*` (prefix `:62`); katalog önbelleği (`:81-109`); `api.py:487`, `:509` ile kayıtlı — `/api` altında olduğu için `nginx.conf:116` `location /api` yeter, nginx istisnası YOK |
