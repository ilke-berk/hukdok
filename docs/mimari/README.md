...
| [`veri-teslim-hatti.md`](veri-teslim-hatti.md) | Veri ekibinin teslim paketleri: admin panelinden yükleme → `aktarim_teslimleri` defteri ve durum makinesi → kapı eşikleri → elle "Uygula" → rapor dizininde cevap dosyaları (SharePoint teslim klasörü yolu 17.09.2026'da kalktı); `Düzeltme_Logu`/`DEGER_HAVUZLARI`/kapsam sayfaları, log sözleşmesi, açık kalemler. Veri ekibine verilen dış sözleşme [`docs/veri-teslim/SOZLESME.md`](../veri-teslim/SOZLESME.md) |
| [`bildirimler.md`](bildirimler.md) | Uygulama içi bildirimler: üreticiler (belge işlendi / süre / duruşma / veri teslim), alıcı kuralı (sorumlu avukat + kopya alıcılar − yükleyen, allowlist), tebliğ tarihinin belge işlemeden aşama kararına yazılması, 06:00 taraması, retention, yetki |
| [`raporlama.md`](raporlama.md) | Raporlama modülü (yalnız yönetici, test aşaması): kayıt defteri → `RaporTanimi` doğrulama → Core sorgu → önizleme / Excel-CSV export + `report_runs` koşu logu ve saklanan çıktı → favori şablonlar; AI asistan (`/chat` NDJSON, `rapor_asistani` anahtarı, K6 tek doğrulama); env'ler, güvenlik modeli, planla farklar, bilinen sınırlar |
