...
## 8. Açık kalanlar (03.09.2026, G114 · 08.09.2026 G148: tenant ayrımı şerhi)
...
| Tenant ayrımı (08.09) | **Neden:** veri ekibinin 06.09 e-postası §13 — `03_VERI_TESLIM/gelen` görünmüyor, davet ulaşmadı. Prod arşiv site'ı LexisBio tenant'ında (30.07 DB kopyasındaki 176 belge URL'sinin tamamı orada — G147 görev kaydı), ekip hesabı Hanyaloğlu tenant'ında → 03.09 paylaşımı tenant'lar arası **misafir davetine** düştü. **Karar (kullanıcı, 08.09):** yalnız teslim hattı (gözcü listeleme + indirme, cevap paketi yükleme) Hanyaloğlu tenant'ındaki `hukdok_arsiv` site'ına taşındı; arşiv, ofis-no sayacı, `log` listesi, belge URL'leri, e-posta ve hukukbot export'u LexisBio'da KALDI (arşiv taşınmaz). **Kod:** G147 — `teslim` config'i, `TESLIM_SHAREPOINT_*` dörtlüsü, biri boşsa arşiv kimliğine düşüş (`sharepoint/auth_graph.py:25-29`, `:136-143`; `services/teslim_kutusu.py:178`); klasör adları site'tan bağımsız. **Dokümanlar:** G148 (bu satır; `dis-bagimliliklar.md` §2, `kimlik-ve-token.md` §4, `deploy-ve-altyapi.md` §1, `SOZLESME.md` §1) | G147 TAMAM (main'de, deploy edilmedi); kurulum alt satırda |
| Prod kurulumu | **Yeni site'a göre (G147 sonrası):** (1) Hanyaloğlu tenant'ındaki `hukdok_arsiv` site'ı → `Belgeler` drive'ında `03_VERI_TESLIM/gelen` (veri ekibine düzenleme) + `03_VERI_TESLIM/cevap` (görüntüleme) klasörleri; aynı tenant → davet gerekmez, bağlantı 06.09 e-postasına cevapta; (2) o tenant'taki uygulama kaydının app-only site izni (`Sites.ReadWrite.All` ya da `Sites.Selected` + site izni) Entra'da teyit, secret bitiş tarihi `TESLIM_SHAREPOINT_CLIENT_SECRET_EXPIRES_AT`; (3) prod `.env`'e `TESLIM_SHAREPOINT_TENANT_ID` / `CLIENT_ID` / `CLIENT_SECRET` / `SITE_URL` + `SHAREPOINT_FOLDER_TESLIM_NAME` (+ isteğe bağlı `TESLIM_KAPI_*`), `docker compose up -d` (recreate, stack'in tamamı); (4) admin panelden anahtar (ilk teslim daima `inceleme_bekliyor`); (5) LexisBio site'ındaki eski `03_VERI_TESLIM` (03.09 lokal testinin izi) silinir ya da bırakılır. Sıra ve satır referansları `veri-teslim-hatti.md` §9 | insan |
