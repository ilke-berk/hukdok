...
- [ ] **1-C** · `deploy.sh` yeniden yazımı (build→up, ff-only, imaj etiketleme, rollback.sh, deploy öncesi pg_dump, gerçek /healthz kapısı) + sunucuda elle prova → **Deploy #2** — kod + LOKAL prova tamam (deploy uçtan uca yeşil, rollback dahil); sunucu provası + Deploy #2 bekliyor; prova öncesi sunucuda `git checkout -- backend/docker-entrypoint.sh deploy.sh` (mode-only kirlilik) + `rm docker-compose.override.yml` gerekli
