...
/**
 * G121 — Hizmet Türü liste filtresi. Sözleşme (G119 ile ortak, dondurulmuş):
 * query param adı `hizmet_turu`, değeri listenin ADIdır (ör. "Lexis Rapor").
 * useCases.olayTuru testinin ikizi: param mekaniği hook seviyesinde doğrulanır,
 * CaseList seçimi olduğu gibi geçirir.
 */
describe("useCases — G121 hizmet_turu filtresi", () => {
  ...
  it("seçim yapılınca liste isteğine hizmet_turu param'ı eklenir (değer = listenin ADI)", async () => {
    authRequestMock.mockResolvedValue(jsonResponse([], { totalCount: "0" }));

    await act(async () => {
      await api().getCases({ limit: 15, hizmetTuru: "Lexis Rapor" });
    });

    expect(authRequestMock).toHaveBeenCalledTimes(1);
    const url = String(authRequestMock.mock.calls[0][0]);
    const params = new URLSearchParams(url.split("?")[1] ?? "");
    expect(params.get("hizmet_turu")).toBe("Lexis Rapor");
    // Diğer parametreler bozulmaz
    expect(params.get("limit")).toBe("15");
  });

  it("olay_turu ile birlikte verilince ikisi de ayrı param olarak gider", async () => {
    authRequestMock.mockResolvedValue(jsonResponse([], { totalCount: "0" }));

    await act(async () => {
      await api().getCases({ olayTuru: "Belgeleme Olayı", hizmetTuru: "Danışmanlık" });
    });
    ...
    expect(params.get("hizmet_turu")).toBe("Danışmanlık");
  });

  it("'Tümü' (ALL) seçiliyken ve hiç verilmediğinde hizmet_turu param'ı GÖNDERİLMEZ", async () => {
    authRequestMock.mockResolvedValue(jsonResponse([], { totalCount: "0" }));

    await act(async () => {
      await api().getCases({ hizmetTuru: "ALL" });
      await api().getCases({});
    });

    expect(authRequestMock).toHaveBeenCalledTimes(2);
    for (const call of authRequestMock.mock.calls) {
      expect(String(call[0])).not.toContain("hizmet_turu");
    }
  });
});
