...
# Tek kimlik seti: `default` = arşiv (LexisBio tenant'ı: belge arşivi, ofis-no sayacı,
# `log` listesi, e-posta, hukukbot export'u). G147'nin ikinci seti (`teslim`,
# `TESLIM_SHAREPOINT_*` — Hanyaloğlu tenant'ındaki veri teslim klasörü) 17.09.2026'da
# teslim klasörü yoluyla birlikte KALDIRILDI; `config_type` parametresi kalıcı (tek değer).
CONFIG_DEFAULT = "default"
CONFIG_TYPES = (CONFIG_DEFAULT,)
#: config → env öneki (`<önek>TENANT_ID` / `CLIENT_ID` / `CLIENT_SECRET`).
_ENV_PREFIX = {CONFIG_DEFAULT: "SHAREPOINT_"}
...
def check_client_secret_expiry(
    ...
    `env_name` (varsayılan `SHAREPOINT_CLIENT_SECRET_EXPIRES_AT`) ISO tarih, örn. 2027-03-15:
...
def _read_credentials(config_type: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    WARNING basar (eksik env zaten `_get_msal_app`'ta RuntimeError olur).
    """
...
def _get_msal_app(config_type: str = CONFIG_DEFAULT) -> msal.ConfidentialClientApplication:
    ...
    config_type: yalnız `"default"` (arşiv — `SHAREPOINT_*`). `_MSAL_APPS` anahtarı
    config_type'tır; `force_refresh` yolu (`get_graph_token`) config'e sadık kalır.
    """
    global _MSAL_APPS
...
def get_graph_token(config_type: str = CONFIG_DEFAULT, force_refresh: bool = False) -> str:
    ...
    config_type: 'default' (arşiv; tek kimlik seti)
