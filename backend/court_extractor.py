...
def _format_match(match: re.Match) -> str:
    il   = (match.group("il")   or "").strip().title()
    sira = (match.group("sira") or "").strip()
    tur  = (match.group("tur")  or "").strip().title()

    # Türkçe title-case düzeltmeleri
    for tr, fixed in [("İ", "İ"), ("Ş", "Ş"), ("Ç", "Ç"), ("Ğ", "Ğ"), ("Ü", "Ü"), ("Ö", "Ö")]:
        pass  # Python title() zaten yeterli
    ...
    # Daire varsa (bütün match'ten alınmaz, substr'dan almalıyız — basit fallback)
    full_str = match.group(0)
    daire_m = re.search(r"(\d+)\.\s*((?:İDARİ\s+)?(?:HUKUK\s+|CEZA\s+)?DAİRESİ)", full_str, re.IGNORECASE)
    if daire_m:
        parts.append(f"{daire_m.group(1)}. {daire_m.group(2).strip().title()}")
