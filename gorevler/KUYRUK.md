...
- [ ] G196 | bant:backend | bagimli:- | Dava durumu yazma yollarında üçlü kapısı: `add_case` status'u `normalize_case_status`'tan geçirir (eski değer → üçlü + aşama); üçlü dışı değer `add_case`/`update_case`/`update_case_tracking`'te veritabanına gitmeden 400 (ERROR log yok) — G195 CHECK sonrası CheckViolation → 500/404 açığı (kullanıcı kararı 14.09)
