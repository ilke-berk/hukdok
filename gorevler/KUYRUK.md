...
- [ ] G192 | bant:backend | bagimli:G190,G191 | Düşük etkili: `offset` tavanı 422; `routes/cases.py` ilişki + `routes/clients.py` poliçe N+1 → `in_()`; `upload_queue._scan_once` vade filtresi SQL'de (parçalı index); PK ikizi `ix_<tablo>_id` → `_DUSURULECEK_INDEXLER` (`models.py` DEĞİŞMEZ) (D8, D10, D11, D15) — 14.09 kullanıcı kararı: D10 seçenek B, D9 G195'e ayrıldı
