...
- [ ] G175 | bant:frontend | bagimli:G173,G174 | Sohbet öncelikli rapor ekranı: yerleşim — SourceCards/QuickFilters/ColumnSheet/ColumnPicker kalkar, AssistantBar → TanimSeridi → sayaç/şablon(kompakt)/indirme satırı → PreviewTable; anahtar kapalı kartı + şerit yedek; sayfa testleri yeni kimliklerle | BLOKE(KAPI: test butunlugu ihlali (Silinen test dosyasi: frontend/src/components/reports/ColumnSheet.test.tsx (286 satir); Silinen test dosyasi: frontend/src/components/reports/QuickFilters.test.tsx (864 satir)))
