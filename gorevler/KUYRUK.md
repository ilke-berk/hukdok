...
- [ ] G195 | bant:backend | bagimli:G192 | `cases.status` CHECK NOT VALID idempotent op (değer listesi `CASE_STATUSES`'ten; VALIDATE prod ölçümüne bağlı ayrı görev) — 14.09 kullanıcı kararı: `test_perf_olcum.py::olcum_db` TEMYIZ satırı kısıttan önce kurulacak şekilde değişebilir (D9)
