...
- [x] G184 | bant:frontend | bagimli:G182 | `useConfigList(key)` liste başına hook (önbellek `useConfig` ile ortak, dönüş sözleşmesi değişmez) + QueryClient `refetchOnWindowFocus:false`; odakta 32 istek testi (F3 hook katmanı)
- [x] G189 | bant:backend | bagimli:G183 | Trigram index'ler gerçek kolonlara: `_TRGM_INDEXES` += `case_foys.tku_no/sistem_no/onceki_tracking_no`; boş legacy `idx_cases_tku_no_trgm`/`idx_cases_sistem_no_trgm` (+ btree) sözlükten çıkar + `_DUSURULECEK_INDEXLER`; `uq_cases_sistem_no` dokunulmaz; karar 018 eki (D2)
- [x] G185 | bant:frontend | bagimli:G184 | Ağır tüketiciler `useConfigList`'e: Index, CaseList, CaseDetails, NewCase, NewClient, EmailModal, IntakeReviewStep, QuickCaseModal — monte edilince yalnız kullandığı `/api/config/*` uçları çağrılır testi; AdminPage dokunulmaz (F3 tüketiciler)
- [x] G187 | bant:frontend | bagimli:G182 | AdminPage: 13 "query → local state" effect'i kalkar, listeler render'da türetilir, yalnız sürükleme/kaydedilmemiş düzenleme state'te; aynı içerikle refetch'te kart yeniden render olmaz testi (F4)
...
- [x] G194 | bant:backend | bagimli:- | Teslim cevap yüklemesi: `cevap_yukle` SharePoint yükleme döngüsünden ÖNCE transaction'ı kapatır, sonra teslimi yeniden okuyup yazar — `idle_in_transaction_session_timeout` açılınca commit düşmez (G191 BLOKE kaydındaki kapsam dışı bulgu; kullanıcı kararı 14.09: G191'den önce)
- [x] G191 | bant:backend | bagimli:G189,G194 | Bağlantı/sunucu ayarları: `_build_connect_args` += `idle_in_transaction_session_timeout` (60 s) + `lock_timeout` (5 s) env'li, migrate.py muaf; compose postgres `command:` effective_cache_size=384MB, random_page_cost=1.1, pg_stat_statements preload + tolerant CREATE EXTENSION; kilit senaryosu dbtest; recreate notu (D5, D6) — 14.09 kullanıcı kararı: `test_faz3_e_hardening.py:129` bekçi metni güncellenebilir
- [x] G186 | bant:frontend | bagimli:G185 | Rerender: CaseDetails `DocCard` modül düzeyine (Select DOM düğümü korunur testi), Index lazy state init + effect bağımlılığı primitif, NewCase düzenleme modu tek render (F5, F8, F9, F10)
- [x] G188 | bant:frontend | bagimli:G184 | theme-provider try/catch + ErrorBoundary içine (storage kapalıyken beyaz ekran yok testi); YetkiBelgesi TC hiç önbelleklenmez, sicil/ad sessionStorage `:v1` try/catch'li, eski anahtar silinir — KVKK maddesi (F6, F7)
- [x] G192 | bant:backend | bagimli:G190,G191 | Düşük etkili: `offset` tavanı 422; `routes/cases.py` ilişki + `routes/clients.py` poliçe N+1 → `in_()`; `upload_queue._scan_once` vade filtresi SQL'de (parçalı index); PK ikizi `ix_<tablo>_id` → `_DUSURULECEK_INDEXLER` (`models.py` DEĞİŞMEZ) (D8, D10, D11, D15) — 14.09 kullanıcı kararı: D10 seçenek B, D9 G195'e ayrıldı
- [x] G195 | bant:backend | bagimli:G192 | `cases.status` CHECK NOT VALID idempotent op (değer listesi `CASE_STATUSES`'ten; VALIDATE prod ölçümüne bağlı ayrı görev) — 14.09 kullanıcı kararı: `test_perf_olcum.py::olcum_db` TEMYIZ satırı kısıttan önce kurulacak şekilde değişebilir (D9)
- [x] G193 | bant:docs | bagimli:G186,G187,G188,G192,G195 | Performans turu dokümantasyonu: CLAUDE.md arama + nginx paragrafları, deploy-ve-altyapi (perf_olcum, compose parametreleri, env'ler, pg_stat_statements), genel-bakis (kod bölme, useConfigList), karar 018 nihai index tablosu, denetim raporuna "uygulamada değişti" şerhi — hepsi koddan doğrulanmış
- [x] G196 | bant:backend | bagimli:- | Dava durumu yazma yollarında üçlü kapısı: `add_case` status'u `normalize_case_status`'tan geçirir (eski değer → üçlü + aşama); üçlü dışı değer `add_case`/`update_case`/`update_case_tracking`'te veritabanına gitmeden 400 (ERROR log yok) — G195 CHECK sonrası CheckViolation → 500/404 açığı (kullanıcı kararı 14.09)
- [ ] G197 | bant:backend | bagimli:G196 | Bayat metin ve atıflar: `perf_olcum.py:413` "CHECK" → "VALIDATE ancak 0 iken" (+ deploy-ve-altyapi şerhi), `nginx.conf:44` "host konfigi repo DIŞINDA" yorumu yanlış, yaşayan dokümanlarda `nginx.conf:N` atıfları koddan doğrulanır, CLAUDE.md test sayıları koşularak güncellenir (kullanıcı kararı 14.09)
...
- [x] G179 | bant:backend | bagimli:G178 | `scripts/ekip_cevabi_1209.py`: Ek-3 › 01 çift kart birleştirme (36 + H-15496 föy taşıma) + §3/Ek-2 › 03 sabit çiftler + Ek-3 › 02 8 föy taşıma + Ek-2 › 06/§3 18 kart düzeltmesi + 4 hatalı kopya (belgeler gerçek karta) ve 4 deneme kartı kapatma — 13.09 gece kullanıcı `--apply` koştu, LOKAL DB'de UYGULANDI (canlı kart 14.363 → 14.315: 40 birleşme + 8 kapatma; föy 8.395; tarihçe 46 + 40; yedek `pre_g179_20260913.dump`), prod'a DEĞİL; 1 ret (14321/14322 esas farklı) + 28 "Rücu" kartı ekibe soru
- [x] G181 | bant:backend | bagimli:- | `scripts/dava_kartlari_listesi.py`: ekibe giden dava kartları listesinin üreticisi repoya alındı (11.09'da oturum geçici dosyasındaydı) — 89 sütun birebir + `İlişkili kartlar`, Açıklama'da dışa aktarım saati ve kart modeli; 13.09 listesi (14.338 kart · 8.395 föy · 23 ilişki) bununla üretildi
- [x] G180 | bant:backend | bagimli:G179 | `scripts/birlesik_kart_ayir.py`: Ek-3 › 03 + Ek-3 › 02 SORU kartları — farklı tür/esas föy grubu YENİ karta (künye/taraflar föyün `ham_veri`sinden, ofis no mevcut kural, klasör no paylaşılır), `case_relations` AYRISTIRILAN bağı, iki kartta tarihçe; kartlar TKU birleştirmesinden gelmiyor (föylerde `onceki_tracking_no` yok → geri getirme yolu gerekmedi, izi olan föy RET). Kullanıcı kararı 13.09 (iki kez teyit): ekibin modeli. Lokal kuru koşu: 30 kart → 23 ayrıldı / 23 yeni kart · 6 atlandı (G179 sonrası tek grup) · 1 ret (15291 G179'da birleşmiş); 13.09 gece kullanıcı `--apply` koştu, LOKAL'de UYGULANDI (canlı kart 14.315 → 14.338, 23 ilişki, tarihçe 46), prod'a DEĞİL
