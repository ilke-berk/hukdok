...
- [ ] G192 | bant:backend | bagimli:G190,G191 | Düşük etkili: `offset` tavanı 422; `routes/cases.py` ilişki + `routes/clients.py` poliçe N+1 → `in_()`; `upload_queue._scan_once` vade filtresi SQL'de (parçalı index); PK `index=True` ikizleri kalkar + `_DUSURULECEK_INDEXLER`; `cases.status` CHECK NOT VALID idempotent op (değer listesi `CASE_STATUSES`'ten; VALIDATE prod ölçümüne bağlı ayrı görev) (D8-D11, D15) | BLOKE(testi degistirmeden gecilemedi - gorev tanimi gozden gecirilmeli)
